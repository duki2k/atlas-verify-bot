...
class AtlasVerifyBot(commands.Bot):
    ...
    async def setup_hook(self) -> None:
        ...
        async def only_admin_channel(interaction: discord.Interaction) -> bool:
            ...
            if settings.admin_channel_id and interaction.channel_id != settings.admin_channel_id:
                raise app_commands.CheckFailure(
                    f"❌ Comandos só podem ser usados em <#{settings.admin_channel_id}>."
                )
            return True
    ...
    async def on_app_command_error(self, interaction: discord.Interaction, error: app_commands.AppCommandError) -> None:
        # ✅ mensagem limpa quando alguém tentar fora do canal
        if isinstance(error, app_commands.CheckFailure):
            msg = str(error) or "❌ Você não pode usar esse comando aqui."
            try:
